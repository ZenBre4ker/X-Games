...
namespace DotsAndBoxes
{
    public partial class VariablesContainer : MonoBehaviour
    {
        ...
        public void setNumberOfPlayers(int _myId,int _localPlayers, int _networkPlayers, int _numberOfPlayers)
        {
            numberOfPlayers = _numberOfPlayers;
            localPlayers = _localPlayers;
            networkPlayers = _networkPlayers;
            AIPlayers = numberOfPlayers-_localPlayers-_networkPlayers;

            playerInputTypes = new inputTypes[numberOfPlayers];
            
            if (_myId < 2)
            {
                Debug.Log("I am host my Id is {_myId}");
                int counter = 0;
                for (int i = 0; i < localPlayers; i++)
                {
                    Debug.Log($"{counter}-th player is localPlayer");
                    playerInputTypes[counter] = inputTypes.Mouse;
                    counter++;
                }
                for (int i = 0; i < networkPlayers; i++)
                {
                    Debug.Log($"{counter}-th player is NetworkPlayer");
                    playerInputTypes[counter] = inputTypes.NetworkPlayer;
                    counter++;
                }
                for (int i = 0; i < AIPlayers; i++)
                {
                    Debug.Log($"{counter}-th player is AI");
                    playerInputTypes[counter] = inputTypes.AI;
                    counter++;
                }
            } else
            {
                Debug.Log($"I am client my Id is {_myId}");
                for (int i = 0; i < numberOfPlayers; i++)
                {
                    if (i != localPlayers + _myId - 2)
                    {
                        Debug.Log($"{i}-th player is NetworkPlayer");
                        playerInputTypes[i] = inputTypes.NetworkPlayer;
                    } else
                    {
                        Debug.Log($"{i}-th player is me");
                        playerInputTypes[i] = inputTypes.Mouse;
                    }
                }
            }
        }
