...
namespace Networking
{
    public class Server : MonoBehaviour
    {
        ...
        public void StartServer( ServerClientManager manager)
        {
            ...
            myNetworkInfo = manager.myNetworkInfo;

            clients = new Dictionary<int, Client>();

            Debug.Log("Server is starting...");
            tcpListener = new TcpListener(IPAddress.Any, myNetworkInfo.port);
            tcpListener.Start();
            tcpListener.BeginAcceptTcpClient(new AsyncCallback(TCPConnectCallback), null);

            udpListener = new UdpClient(myNetworkInfo.port);
            udpListener.BeginReceive(UDPReceiveCallback, null);
            Debug.Log($"Server started on Port {myNetworkInfo.port}.");
        ...
        {
            numberOfClients += 1;
            myNetworkInfo.numberOfClients += 1;
